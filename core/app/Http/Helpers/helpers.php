...
{
  // Handle missing product or price property
  if (is_null($product) || !isset($product->price)) {
    return 0; // Assuming a default price of 0, adjust as needed
  }

  $discountPrice = showDiscountPrice($product->price, $product->discount, $product->discount_type);

  // Apply today's deal discount conditionally
  if ($product->today_deals === Status::YES) {
    $general = gs(); // Assuming `gs()` retrieves general discount information
    $discountPrice = showDiscountPrice($discountPrice, $general->discount, $general->discount_type);
  }

  // Ensure non-negative price efficiently
  return max(0, $discountPrice);
}

function showDiscountPrice($price, $discount, $discount_type)
{
  // Early return for zero discount for clarity
  if ($discount === 0) {
    return $price;
  }
